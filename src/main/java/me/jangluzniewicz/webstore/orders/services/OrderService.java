...
public class OrderService implements IOrder {
    ...
    public OrderService(OrderRepository orderRepository, IUser userService, IProduct productService,
                        IOrderStatus orderStatusService, OrderMapper orderMapper, UserMapper userMapper,
                        OrderStatusMapper orderStatusMapper, OrderItemMapper orderItemMapper,
    ...
    public Long createNewOrder(@NotNull OrderRequest orderRequest) {
        Order order = Order.builder()
                ...
                .customer(userService.getUserById(orderRequest.getCustomerId()).orElseThrow(
                        () -> new NotFoundException("User with id " + orderRequest.getCustomerId() + " not found")))
                .status(orderStatusService.getOrderStatusById(orderRequest.getStatusId()).orElseThrow(
                        () -> new NotFoundException("Order status with id "
                                + orderRequest.getStatusId() + " not found")))
                .rating(Rating.builder()
                        .rating(orderRequest.getRating().getRating())
                        .description(orderRequest.getRating().getDescription())
                        .build())
                .items(orderRequest.getItems().stream().map(orderItemRequest -> OrderItem.builder()
                        .product(productService.getProductById(orderItemRequest.getProductId()).orElseThrow(
                                () -> new NotFoundException("Product with id "
    ...
    public List<Order> getFilteredOrders(@Min(1) Long statusId, LocalDateTime orderDateAfter,
        ...
        if (statusId != null && orderDateAfter != null && orderDateBefore != null) {
            orders = orderRepository.findAllByStatusIdAndOrderDateBetweenOrderByOrderDateAscIdAsc(
                    statusId, orderDateAfter, orderDateBefore, pageable).map(orderMapper::fromEntity);
        } else if (statusId != null) {
            orders = orderRepository.findAllByStatusIdOrderByOrderDateAscIdAsc(statusId, pageable)
                    .map(orderMapper::fromEntity);
        } else if (orderDateAfter != null && orderDateBefore != null) {
            orders = orderRepository.findAllByOrderDateBetweenOrderByOrderDateAscIdAsc(
                    orderDateAfter, orderDateBefore, pageable).map(orderMapper::fromEntity);
        } else if (orderDateAfter != null || orderDateBefore != null) {
            throw new IllegalArgumentException("Both orderDateAfter and orderDateBefore must be specified");
        } else {
    ...
    public Long updateOrder(@NotNull @Min(1) Long id, @NotNull OrderRequest orderRequest) {
        ...
        orderEntity.setCustomer(userMapper.toEntity(userService.getUserById(orderRequest.getCustomerId())
                .orElseThrow(() -> new NotFoundException("User with id "
                        + orderRequest.getCustomerId() + " not found"))));
        orderEntity.setStatus(orderStatusMapper.toEntity(orderStatusService.getOrderStatusById(
                orderRequest.getStatusId()).orElseThrow(() -> new NotFoundException("Order status with id "
                + orderRequest.getStatusId() + " not found"))));
    ...
    @Override
    @Transactional
    public Long changeOrderStatus(@NotNull @Min(1) Long id,
                                  @NotNull ChangeOrderStatusRequest changeOrderStatusRequest) {
        OrderEntity orderEntity = orderRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("Order with id " + id + " not found"));
        orderEntity.setStatus(orderStatusMapper.toEntity(orderStatusService.getOrderStatusById(
                changeOrderStatusRequest.getOrderStatusId()).orElseThrow(
                () -> new NotFoundException("Order status with id "
